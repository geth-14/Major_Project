...
print(np.max(df.iloc[0:1013,1]), np.min(df.iloc[0:1013,1]))
train_set = df.iloc[0:995,1:2].values
test_set = df.iloc[995:1013,1:2].values
scaler = MinMaxScaler(feature_range = (0,1))
train_set_scaled = scaler.fit_transform(train_set)

X_train = []  
Y_train = [] 
for i in range(60, 995):  
...
tot = pd.concat((df.iloc[0:995,:]['open'], df.iloc[995:1013,:]['open']), axis=0)
test_ip = tot[len(tot) - len(df.iloc[995:1013,:]) - 60:].values  
...
Actual = np.array(df.iloc[995:1013,1]) 
Predicted = predict  
